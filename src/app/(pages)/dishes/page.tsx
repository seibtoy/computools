...
import { dishesProducts } from '@/mocks';
import { shuffleArray } from '@/utils';

export default function DishesPage() {
  const hotProducts = dishesProducts.filter((product) => product.isHot);
  const shuffledHotProducts = shuffleArray(hotProducts);

  const topSoldProduct = dishesProducts.filter((product) => product.isTopSeller);
  const shuffledTopSellers = shuffleArray(topSoldProduct);

  const popularProducts = dishesProducts.filter((product) => product.isPopular);
