...
import { gamingProducts } from '@/mocks';
import { shuffleArray } from '@/utils';

export default function GamingProductPage() {
  const hotProducts = gamingProducts.filter((product) => product.isHot);
  const shuffledHotProducts = shuffleArray(hotProducts);

  const topSoldProduct = gamingProducts.filter((product) => product.isTopSeller);
  const shuffledTopSellers = shuffleArray(topSoldProduct);

  const popularProducts = gamingProducts.filter((product) => product.isPopular);
