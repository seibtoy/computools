...
import { largeAppliancesProducts } from '@/mocks';
import { shuffleArray } from '@/utils';

export default function LargeAppliancesPage() {
  const hotProducts = largeAppliancesProducts.filter((product) => product.isHot);
  const shuffledHotProducts = shuffleArray(hotProducts);

  const topSoldProduct = largeAppliancesProducts.filter((product) => product.isTopSeller);
  const shuffledTopSellers = shuffleArray(topSoldProduct);

  const popularProducts = largeAppliancesProducts.filter((product) => product.isPopular);
