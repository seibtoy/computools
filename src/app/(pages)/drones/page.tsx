...
import { dronesProducts } from '@/mocks';
import { shuffleArray } from '@/utils';

export default function DronesPage() {
  const hotProducts = dronesProducts.filter((product) => product.isHot);
  const shuffledHotProducts = shuffleArray(hotProducts);

  const topSoldProduct = dronesProducts.filter((product) => product.isTopSeller);
  const shuffledTopSellers = shuffleArray(topSoldProduct);

  const popularProducts = dronesProducts.filter((product) => product.isPopular);
