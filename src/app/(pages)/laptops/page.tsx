...
import { laptopsProducts } from '@/mocks';
import { shuffleArray } from '@/utils';

export default function LaptopPage() {
  const hotProducts = laptopsProducts.filter((product) => product.isHot);
  const shuffledHotProducts = shuffleArray(hotProducts);

  const topSoldProduct = laptopsProducts.filter((product) => product.isTopSeller);
  const shuffledTopSellers = shuffleArray(topSoldProduct);

  const popularProducts = laptopsProducts.filter((product) => product.isPopular);
