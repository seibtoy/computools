...
import { audioProducts } from '@/mocks';
import { shuffleArray } from '@/utils';

export default function AudioProductsPage() {
  const hotProducts = audioProducts.filter((product) => product.isHot);
  const shuffledHotProducts = shuffleArray(hotProducts);

  const topSoldProduct = audioProducts.filter((product) => product.isTopSeller);
  const shuffledTopSellers = shuffleArray(topSoldProduct);

  const popularProducts = audioProducts.filter((product) => product.isPopular);
