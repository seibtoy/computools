...
import { computersProducts } from '@/mocks';
import { shuffleArray } from '@/utils';

export default function ComputersPage() {
  const hotProducts = computersProducts.filter((product) => product.isHot);
  const shuffledHotProducts = shuffleArray(hotProducts);

  const topSoldProduct = computersProducts.filter((product) => product.isTopSeller);
  const shuffledTopSellers = shuffleArray(topSoldProduct);

  const popularProducts = computersProducts.filter((product) => product.isPopular);
