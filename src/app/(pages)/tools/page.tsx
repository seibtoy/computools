...
import { toolsProducts } from '@/mocks';
import { shuffleArray } from '@/utils';

export default function ToolsPage() {
  const hotProducts = toolsProducts.filter((product) => product.isHot);
  const shuffledHotProducts = shuffleArray(hotProducts);

  const topSoldProduct = toolsProducts.filter((product) => product.isTopSeller);
  const shuffledTopSellers = shuffleArray(topSoldProduct);

  const popularProducts = toolsProducts.filter((product) => product.isPopular);
